...
class StateGoalNetwork(nn.Module):
    ...
    goal_dim: int
    dim_out: int = 1
    layers: list = (400, 300) # from their paper
    state_embedding: nn.Module = None # should be None (as from their paper)
    goal_embedding: nn.Module = None
    freeze_embeddings: bool = False # irrelevant as we don't use embeddings

    @nn.compact
    def __call__(self, state, goal):
        # Default to identity if no embedding provided
        state_emb = self.state_embedding(state) if self.state_embedding else state
        goal_emb = self.goal_embedding(goal) if self.goal_embedding else goal

        if self.freeze_embeddings:
            state_emb = jax.lax.stop_gradient(state_emb)
            goal_emb = jax.lax.stop_gradient(goal_emb)

        x = jnp.concatenate([state_emb, goal_emb], axis=-1)
...
def class_select(logits, target):
    # logits: [batch, num_classes], target: [batch]
    batch_idx = jnp.arange(logits.shape[0])
    return logits[batch_idx, target]

def cross_entropy_with_weights(logits, target, weights=None, label_smoothing=0):
    """Compute cross entropy loss with optional weights and label smoothing."""
    assert logits.ndim == 2
    target = jnp.squeeze(target) if target.ndim == 2 else target
    assert target.ndim == 1
    
    loss = jax.nn.logsumexp(logits, axis=1) - (1-label_smoothing) * class_select(logits, target) - label_smoothing * jnp.mean(logits, axis=1)
...
class IndependentDiscretizedStochasticGoalPolicy(nn.Module):
    """Policy network that outputs discrete actions for each dimension independently."""
    state_dim: int  # Dimension of state space
    goal_dim: int   # Dimension of goal space
    action_size: int
    granularity: int = 3 # default from their github
    n_dims: int
    ...
    def setup(self):
        self.net = StateGoalNetwork(
            ...
            layers=self.layers,
            # layers=[self.network_width] * self.network_depth
        )

    def flattened(self, tensor):
        """Convert multi-dimensional tensor to flattened indices."""
        multipliers = self.granularity ** jnp.arange(self.n_dims)
        flattened = jnp.sum(tensor * multipliers[None, :], axis=1)
        return jnp.array(flattened, dtype=jnp.int32)
    
    def unflattened(self, tensor):
        """Convert flattened indices back to multi-dimensional tensor."""
        digits = []
        output = tensor
    ...
    @nn.compact
    def __call__(self, obs, goal, horizon=None):
        return self.net(obs, goal)

    def act_vectorized(self, params, obs, goal, horizon=None, greedy=False, noise=0, key=None):
        """Sample actions from the policy."""
        if key is None:
            key = jax.random.PRNGKey(0)
        
        # Split key for network forward pass
        key, subkey = jax.random.split(key)
        logits = self.apply(params, obs, goal, rngs={'dropout': subkey})
        logits = jnp.reshape(logits, (-1, self.n_dims, self.granularity))
        noisy_logits = logits * (1 - noise)
        probs = jax.nn.softmax(noisy_logits, axis=2)

        # Split key for sampling
        key, subkey = jax.random.split(key)
        if greedy:
            samples = jnp.argmax(probs, axis=-1)
        else:
            samples = jax.random.categorical(subkey, logits=noisy_logits, axis=-1)
        
        samples = self.flattened(samples)
        
        if greedy:
            # Split key for random samples and noise mask
            key, subkey1, subkey2 = jax.random.split(key, 3)
            random_samples = jax.random.randint(subkey1, (len(samples),), 0, self.action_size)
            mask = jax.random.uniform(subkey2, (len(samples),)) < noise
            return jnp.where(mask, random_samples, samples)
        return samples
    
    def nll(self, params, obs, goal, actions, horizon=None):
        """Compute negative log likelihood of actions."""
        actions_perdim = self.unflattened(actions)
        actions_perdim = jnp.reshape(actions_perdim, (-1,))

        # Use apply with params
        logits = self.apply(params, obs, goal)
        logits_perdim = jnp.reshape(logits, (-1, self.granularity))
        
        loss_perdim = CrossEntropyLoss(aggregate=None, label_smoothing=0)(logits_perdim, actions_perdim)
        loss = jnp.reshape(loss_perdim, (-1, self.n_dims))
        return jnp.sum(loss, axis=1)
        
    def entropy(self, params, obs, goal, horizon=None):
        """Compute entropy of the policy distribution."""
        # Use apply with params
        logits = self.apply(params, obs, goal)
        logits = jnp.reshape(logits, (-1, self.n_dims, self.granularity))
        probs = jax.nn.softmax(logits, axis=2)
        Z = jax.nn.logsumexp(logits, axis=2)
        return jnp.sum(Z - jnp.sum(probs * logits, axis=2), axis=1)
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}_\
        actorwidth:{args.actor_network_width}_\
        actordepth:{args.actor_depth}_\
    ...
    # Network setup
    # Actor
    actor = Actor(action_size=action_size, network_width=args.actor_network_width, network_depth=args.actor_depth, skip_connections=args.actor_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    ...
    # Critic
    sa_encoder = SA_encoder(network_width=args.critic_network_width, network_depth=args.critic_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    sa_encoder_params = sa_encoder.init(sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]))
    g_encoder = G_encoder(network_width=args.critic_network_width, network_depth=args.critic_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    ...
    if args.capture_vis:
        def render_policy(training_state, save_path):
            ...
            render_path = Path(save_path) / "vis.html"
