...
class Residual8(nn.Module):
    ...
    def __call__(self, x):
        for _ in range(8):
            x = residual_block(x, self.width, self.normalize, self.activation)
        return x
...
class SA_encoder(nn.Module):
    ...
    def __call__(self, s: jnp.ndarray, a: jnp.ndarray):
        ...
        num_residual_blocks = self.network_depth // 4
        num_residual_chunks = num_residual_blocks // 8 #each chunk is 8 residual blocks (Residual8)
        #Residual blocks
        for i in range(num_residual_chunks):
            x = jax.jit(Residual8)(self.network_width, normalize, activation)(x)
