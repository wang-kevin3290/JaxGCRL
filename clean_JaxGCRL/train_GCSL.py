...
class Args:
    ...
    cuda: bool = True
    track: bool = True
    wandb_project_name: str = "GCSL_clean_JaxGCRL_test"
    ...
    last_state_only: int = 0 #THIS IS NOT USED ANYMORE (we always sample as usual)
    conda_env: str = "unspecified"
...
if __name__ == "__main__":
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            #extracting out the states, actions, future states, and goals
            obs = transitions.observation           # expected_shape = batch_size, obs_size + goal_size
            ACTION_demo = transitions.action #We add this in here, needed for the behavioral cloning loss
            state = obs[:, :args.obs_dim]
            future_state = transitions.extras["future_state"]
            goal = future_state[:, args.goal_start_idx : args.goal_end_idx] #SO HERE, IN ACTOR LOSS, WE ARE USING THE FUTURE STATE ON THE SAME TRAJECTORY AS THE GOAL (it itself is the damn goal, yes; goal_start_idx and goal_end_idx are not "goal of this state", but "the indices of this state's observation that corresponds to the goal"; i.e. first 3 indices in humanoid x,y,z coordinates)
            observation = jnp.concatenate([state, goal], axis=1)

            #running inference on the actor -> we actual
            ...
            u_demo = atanh(ACTION_demo)
            log_prob_demo = jax.scipy.stats.norm.logpdf(u_demo, loc=means, scale=stds)
            log_prob_demo -= jnp.log((1.0 - jnp.square(ACTION_demo)) + 1e-6)
            log_prob_demo = log_prob_demo.sum(-1)

            #entropy term -- sample a FRESH action
            u_sample = means + stds * jax.random.normal(key, shape=means.shape, dtype=means.dtype)
            a_sample = nn.tanh(u_sample)
            log_prob_sample = jax.scipy.stats.norm.logpdf(u_sample, loc=means, scale=stds)
            log_prob_sample -= jnp.log((1.0 - jnp.square(a_sample)) + 1e-6)
            log_prob_sample = log_prob_sample.sum(-1)
            ...
            if args.disable_entropy:
                actor_loss = -jnp.mean(log_prob_demo)
            else:
                actor_loss = jnp.mean( jnp.exp(log_alpha) * log_prob_sample - (log_prob_demo) )

            return actor_loss, log_prob_sample
        ...
        (actorloss, log_prob), actor_grad = jax.value_and_grad(actor_loss, has_aux=True)(training_state.actor_state.params, None, training_state.alpha_state.params['log_alpha'], transitions, key)
    ...
    for ne in range(args.num_epochs):
        ...
        if args.checkpoint:
            if ne < 5 or ne >= args.num_epochs - 5 or ne % 10 == 0:
                ...
                params = (training_state.alpha_state.params, training_state.actor_state.params, None)
