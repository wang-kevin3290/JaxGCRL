...
class Args:
    ...
    state_encoder_network_width: int = 256
    goal_encoder_network_width: int = 256
    state_action_network_width: int = 256
    
    state_encoder_depth: int = 4
    goal_encoder_depth: int = 4
    state_action_depth: int = 4
    
    state_encoder_skip_connections: int = 0
    goal_encoder_skip_connections: int = 0
    state_action_skip_connections: int = 0
    
    # Actor parameters
    actor_network_width: int = 1024
    actor_depth: int = 4
    actor_skip_connections: int = 4  # Default to 4 layers per block
    use_relu: int = 0
    resnet: str = "resnet"  # Options: "resnet", "noresnet", "resnetOrig", "identityMapping"
    LOG_STD_MAX: float = 2
    LOG_STD_MIN: float = -5
    ...
    # Runtime computed values that need to be declared
    env_steps_per_actor_step: int = 0
    num_prefill_env_steps: int = 0
    num_prefill_actor_steps: int = 0
    num_training_steps_per_epoch: int = 0
...
aD:{args.actor_depth}_\
aW:{args.actor_skip_connections}_\
sD:{args.state_encoder_depth}_\
gD:{args.goal_encoder_depth}_\
saD:{args.state_action_depth}_\
...
{args.seed}"
    ...
    key, buffer_key, env_key, eval_env_key, actor_key, sa_key, g_key, sym_key, asym_key, memory_bank_key, state_key = jax.random.split(key, 11)
    ...
    sa_encoder = SA_encoder(
        network_width=args.state_action_network_width, 
        network_depth=args.state_action_depth, 
        skip_connections=args.state_action_skip_connections, 
        use_relu=args.use_relu, 
        resnet_type=args.resnet
    )

    g_encoder = G_encoder(
        network_width=args.goal_encoder_network_width, 
        network_depth=args.goal_encoder_depth, 
        skip_connections=args.goal_encoder_skip_connections, 
        use_relu=args.use_relu, 
        resnet_type=args.resnet
    )

    state_encoder = State_encoder(
        network_width=args.state_encoder_network_width, 
        network_depth=args.state_encoder_depth, 
        skip_connections=args.state_encoder_skip_connections, 
        use_relu=args.use_relu, 
        resnet_type=args.resnet
    )
    ...
    if not args.mrn:
        critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": sa_encoder.init(sa_key, np.ones([1, 64]), np.ones([1, action_size])),
                "g_encoder": g_encoder.init(g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx])),
                "state_encoder": state_encoder.init(state_key, np.ones([1, args.obs_dim])),
                "sym": sym_params if args.mrn else None,
                "asym": asym_params if args.mrn else None
            },
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            state_repr = state_encoder.apply(state_encoder_params, state)
            # Then pass encoded state + action to SA encoder
            sa_repr = sa_encoder.apply(sa_encoder_params, state_repr, action)
    ...
    def update_critic(transitions, training_state, key):
        ...
        def critic_loss(critic_params, transitions, key):
            ...
            # Then concatenate with action and pass to SA encoder
            sa_repr = sa_encoder.apply(sa_encoder_params, state_repr, action)
