...
if __name__ == "__main__":
    ...
    key, buffer_key, env_key, eval_env_key, actor_key, sa_key, g_key, sym_key, asym_key, memory_bank_key = jax.random.split(
        key, 10)
    ...
    def make_env(env_id=args.env_id):
        ...
        if env_id == "reacher":
            ...
            if "gen" not in env_id:
                ...
                print(f"maze_layout_name: {maze_layout_name}, generalization_config: {generalization_config}",
                      flush=True)
    ...
    # Network setup
    # Actor
    actor = Actor(action_size=action_size, network_width=args.actor_network_width, network_depth=args.actor_depth,
                  skip_connections=args.actor_skip_connections, use_relu=args.use_relu)
    ...
    def training_step(training_state, env_state, buffer_state, key, t):
        ...
        print(
            f"transitions.observation.shape (after {args.num_episodes_per_env} episodes per env): {transitions.observation.shape}",
            flush=True)
        ...
        print(
            f"transitions.observation.shape (after {args.use_all_batches}, selecting {args.num_sgd_batches_per_training_step} batches): {transitions.observation.shape}",
            flush=True)
    ...
    def training_epoch(
        ...
        training_state, env_state, buffer_state, key), jnp.arange(
