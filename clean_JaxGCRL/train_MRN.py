...
class Args:
    ...
    cuda: bool = True
    track: bool = True
    wandb_project_name: str = "MRN_clean_JaxGCRL_test"
    ...
    mrn_K: int = 8 # if too slow or not training well, try 4
    conda_env: str = "expl-env"
...
if __name__ == "__main__":
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            # qf_pi = -jnp.sqrt(jnp.sum((sa_repr - g_repr) ** 2, axis=-1)) #I believe MRN basically just modified this one thing (how to compute q given sa_repr and g_repr)
            def mrn_distance_component(x: jnp.ndarray, y: jnp.ndarray) -> jnp.ndarray:
                eps = 1e-6
                d = x.shape[-1]
                x_pre, x_suf = x[..., : d // 2], x[..., d // 2 :]
                y_pre, y_suf = y[..., : d // 2], y[..., d // 2 :]
                max_comp = jnp.max(jax.nn.relu(x_pre - y_pre), axis=-1)
                l2_comp = jnp.sqrt(jnp.sum(jnp.square(x_suf - y_suf), axis=-1) + eps)

                return max_comp + l2_comp


            def mrn_distance(x: jnp.ndarray, y: jnp.ndarray, K) -> jnp.ndarray: #try K = 8 first
                x_splits = jnp.array_split(x, K, axis=-1)
                y_splits = jnp.array_split(y, K, axis=-1)
                comps = [mrn_distance_component(xs, ys) for xs, ys in zip(x_splits, y_splits)]
                return jnp.stack(comps, axis=-1).mean(axis=-1)
            
            distance = mrn_distance(sa_repr, g_repr, K=args.mrn_K)
            qf_pi = -distance
