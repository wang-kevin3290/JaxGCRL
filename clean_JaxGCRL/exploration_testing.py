...
if __name__ == "__main__":
    ...
    collector_actor_state = TrainState.create(
        ...
        params=deep_actor.init(collector_actor_key, np.ones([1, obs_size])),
    ...
    collector_sa_encoder_params = deep_sa_encoder.init(collector_sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]))
    ...
    collector_g_encoder_params = deep_g_encoder.init(collector_g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx]))
    ...
    def deterministic_actor_step(training_state, network_id, env, env_state, extra_fields):
        ...
        if network_id == "c":
            means, _ = deep_actor.apply(training_state.collector_actor_state.params, env_state.obs)
        elif network_id == "d":
    ...
    def actor_step(training_state, network_id, env, env_state, key, extra_fields):
        if network_id == "c":
            means, log_stds = deep_actor.apply(training_state.collector_actor_state.params, env_state.obs)
        elif network_id == "d":
