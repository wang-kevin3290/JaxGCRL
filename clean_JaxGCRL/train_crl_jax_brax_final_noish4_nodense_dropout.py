...
def residual_block(x, width, normalize, activation, training):
    identity = x
...
class SA_encoder(nn.Module):
    ...
    use_relu: int = 0
    @nn.compact
    def __call__(self, s: jnp.ndarray, a: jnp.ndarray, training: bool):
...
class G_encoder(nn.Module):
    ...
    use_relu: int = 0
    @nn.compact
    def __call__(self, g: jnp.ndarray, training: bool):
...
if __name__ == "__main__":
    ...
    actor_state = TrainState.create(
        ...
        params=actor.init(actor_key, np.ones([1, obs_size]), training=False),
    ...
    sa_encoder_params = sa_encoder.init(sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]), training=False)
    ...
    g_encoder_params = g_encoder.init(g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx]), training=False)
    ...
    def deterministic_actor_step(training_state, env, env_state, extra_fields, training=False, dropout_key=jax.random.PRNGKey(0)):
        means, _ = actor.apply(training_state.actor_state.params, env_state.obs, training=training, rngs={"dropout": dropout_key})
    ...
    def actor_step(training_state, env, env_state, key, extra_fields, training=False, dropout_key=jax.random.PRNGKey(0)):
        means, log_stds = actor.apply(training_state.actor_state.params, env_state.obs, training=training, rngs={"dropout": dropout_key})
    ...
    def multi_sample_actor_step(training_state, env, env_state, key, K, extra_fields, training=False, dropout_key=jax.random.PRNGKey(0)):
    ...
    def get_experience(training_state, env_state, buffer_state, key):
        ...
        def f(carry, unused_t): #conducts a single actor step in environment
            ...
            current_key, next_key, dropout_key = jax.random.split(current_key, 3)
            if args.expl_actor == 1:
                env_state, transition = actor_step(training_state, env, env_state, current_key, extra_fields=("truncation", "seed"), training=True, dropout_key=dropout_key) #when you get experience, you are training
            elif args.expl_actor == 0:
                env_state, transition = deterministic_actor_step(training_state, env, env_state, extra_fields=("truncation", "seed"), training=True, dropout_key=dropout_key)
            else:
                env_state, transition = multi_sample_actor_step(training_state, env, env_state, current_key, args.expl_actor, extra_fields=("truncation", "seed"), training=True, dropout_key=dropout_key)
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            means, log_stds = actor.apply(actor_params, observation, training=True, rngs={"dropout": dropout_key})
            stds = jnp.exp(log_stds)
            ...
            sa_repr = sa_encoder.apply(sa_encoder_params, state, action, training=True, rngs={"dropout": dropout_key})
            g_repr = g_encoder.apply(g_encoder_params, goal, training=True, rngs={"dropout": dropout_key})
    ...
    def update_critic(transitions, training_state, key):
        ...
        def critic_loss(critic_params, transitions, key):
            ...
            sa_repr = sa_encoder.apply(sa_encoder_params, obs, action, training=True, rngs={"dropout": dropout_key})
            g_repr = g_encoder.apply(g_encoder_params, transitions.observation[:, args.obs_dim:], training=True, rngs={"dropout": dropout_key})
    ...
    if args.capture_vis:
        def render_policy(training_state, save_path):
            ...
            def policy_step(env_state, actor_params):
                means, _ = actor.apply(actor_params, env_state.obs, training=False)
                actions = nn.tanh(means)
