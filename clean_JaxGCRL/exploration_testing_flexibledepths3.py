...
class Args:
    ...
    seed: int = random.randint(1, 1000000) # 16
    torch_deterministic: bool = True
    cuda: bool = True
    track: bool = True
    wandb_project_name: str = "deep_shallow_3networks"
...
class SA_encoder(nn.Module):
    ...
    network_depth: int = 4
    skip_connections: int = 4  # Default to 4 layers per block
    use_relu: int = 0
    ...
    def setup(self):
        ...
        # self.normalize = lambda x: x  # Commented out LayerNorm
...
class Actor(nn.Module):
    ...
    def __call__(self, x):
        ...
        mean = nn.Dense(self.action_size, kernel_init=self.lecun_uniform, bias_init=self.bias_init)(x)
        log_std = nn.Dense(self.action_size, kernel_init=self.lecun_uniform, bias_init=self.bias_init)(x)
...
class TrainingState:
    ...
    collector_actor_state: TrainState
    collector_critic_state: TrainState
    shallow_actor_state: TrainState
    shallow_critic_state: TrainState
    deep_actor_state: TrainState
    deep_critic_state: TrainState
    collector_alpha_state: TrainState
    shallow_alpha_state: TrainState
    deep_alpha_state: TrainState
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}_\
        collectordepth:{args.collector_depth}_\
        deepdepth:{args.deep_depth}_\
        shallowdepth:{args.shallow_depth}_\
        actorskip:{args.actor_skip_connections}_\
        resnet:{args.resnet}_\
        {args.seed}"
    ...
    if args.checkpoint or args.capture_vis:
        ...
        # Create the runs directory and all parent directories
    ...
    key, buffer_key, env_key, eval_env_key, \
        collector_actor_key, collector_sa_key, collector_g_key, \
        shallow_actor_key, shallow_sa_key, shallow_g_key, \
        deep_actor_key, deep_sa_key, deep_g_key, \
        sym_key, asym_key, memory_bank_key = jax.random.split(key, 16)
    ...
    collector_actor = Actor(action_size=action_size, network_width=args.actor_network_width, network_depth=args.collector_depth, skip_connections=args.actor_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    deep_actor = Actor(action_size=action_size, network_width=args.actor_network_width, network_depth=args.deep_depth, skip_connections=args.actor_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    shallow_actor = Actor(action_size=action_size, network_width=args.actor_network_width, network_depth=args.shallow_depth, skip_connections=args.actor_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    # Network setup
    # Actor
    collector_actor_state = TrainState.create(
        apply_fn=collector_actor.apply,
        params=collector_actor.init(collector_actor_key, np.ones([1, obs_size])),
        tx=optax.adam(learning_rate=args.actor_lr)
    )
    deep_actor_state = TrainState.create(
        apply_fn=deep_actor.apply,
        params=deep_actor.init(deep_actor_key, np.ones([1, obs_size])),
        tx=optax.adam(learning_rate=args.actor_lr)
    )
    shallow_actor_state = TrainState.create(
        apply_fn=shallow_actor.apply,
        params=shallow_actor.init(shallow_actor_key, np.ones([1, obs_size])),
        tx=optax.adam(learning_rate=args.actor_lr)
    )

    # Critic
    collector_sa_encoder = SA_encoder(network_width=args.critic_network_width, network_depth=args.collector_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    deep_sa_encoder = SA_encoder(network_width=args.critic_network_width, network_depth=args.deep_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    shallow_sa_encoder = SA_encoder(network_width=args.critic_network_width, network_depth=args.shallow_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)

    collector_sa_encoder_params = collector_sa_encoder.init(collector_sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]))
    deep_sa_encoder_params = deep_sa_encoder.init(deep_sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]))
    shallow_sa_encoder_params = shallow_sa_encoder.init(shallow_sa_key, np.ones([1, args.obs_dim]), np.ones([1, action_size]))
    
    collector_g_encoder = G_encoder(network_width=args.critic_network_width, network_depth=args.collector_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    deep_g_encoder = G_encoder(network_width=args.critic_network_width, network_depth=args.deep_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    shallow_g_encoder = G_encoder(network_width=args.critic_network_width, network_depth=args.shallow_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    
    collector_g_encoder_params = collector_g_encoder.init(collector_g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx]))
    deep_g_encoder_params = deep_g_encoder.init(deep_g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx]))
    shallow_g_encoder_params = shallow_g_encoder.init(shallow_g_key, np.ones([1, args.goal_end_idx - args.goal_start_idx]))
    ...
    if not args.mrn:
        collector_critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": collector_sa_encoder_params, 
                "g_encoder": collector_g_encoder_params
                },
            tx=optax.adam(learning_rate=args.critic_lr),
        )
        deep_critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": deep_sa_encoder_params, 
                "g_encoder": deep_g_encoder_params
                },
            tx=optax.adam(learning_rate=args.critic_lr),
        )   
        shallow_critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": shallow_sa_encoder_params, 
                "g_encoder": shallow_g_encoder_params
                },
            tx=optax.adam(learning_rate=args.critic_lr)
            )
    else:
    ...
    collector_log_alpha = jnp.asarray(0.0, dtype=jnp.float32)
    deep_log_alpha = jnp.asarray(0.0, dtype=jnp.float32)
    shallow_log_alpha = jnp.asarray(0.0, dtype=jnp.float32)
    
    collector_alpha_state = TrainState.create(
        apply_fn=None,
        params={"log_alpha": collector_log_alpha},
        tx=optax.adam(learning_rate=args.alpha_lr),
    )
    deep_alpha_state = TrainState.create(
        apply_fn=None,
        params={"log_alpha": deep_log_alpha},
        tx=optax.adam(learning_rate=args.alpha_lr),
    )
    shallow_alpha_state = TrainState.create(
        apply_fn=None,
        params={"log_alpha": shallow_log_alpha},
        tx=optax.adam(learning_rate=args.alpha_lr),
    )
    ...
    training_state = TrainingState(
        ...
        collector_actor_state=collector_actor_state,
        deep_actor_state=deep_actor_state,
        shallow_actor_state=shallow_actor_state,
        collector_critic_state=collector_critic_state,
        deep_critic_state=deep_critic_state,
        shallow_critic_state=shallow_critic_state,
        collector_alpha_state=collector_alpha_state,
        deep_alpha_state=deep_alpha_state,
        shallow_alpha_state=shallow_alpha_state,
    ...
    def deterministic_actor_step(training_state, network_id, env, env_state, extra_fields):

        if network_id == "c":
            means, _ = collector_actor.apply(training_state.collector_actor_state.params, env_state.obs)
        elif network_id == "d":
            means, _ = deep_actor.apply(training_state.deep_actor_state.params, env_state.obs)
        elif network_id == "s":
            means, _ = shallow_actor.apply(training_state.shallow_actor_state.params, env_state.obs)
    ...
    def actor_step(training_state, network_id, env, env_state, key, extra_fields):
        if network_id == "c":
            means, log_stds = collector_actor.apply(training_state.collector_actor_state.params, env_state.obs)
        elif network_id == "d":
            means, log_stds = deep_actor.apply(training_state.deep_actor_state.params, env_state.obs)
        elif network_id == "s":
            means, log_stds = shallow_actor.apply(training_state.shallow_actor_state.params, env_state.obs)
    ...
    def get_experience(training_state, env_state, buffer_state, key):
        ...
        def f(carry, unused_t): #conducts a single actor step in environment
            ...
            if args.expl_actor == 1:
                env_state, transition = actor_step(training_state, "c", env, env_state, current_key, extra_fields=("truncation", "seed"))
            elif args.expl_actor == 0:
                env_state, transition = deterministic_actor_step(training_state, "c", env, env_state, extra_fields=("truncation", "seed"))
            else:
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, network_id, critic_params, log_alpha, transitions, key):
            ...
            # Now add debug prints with the extracted values
            # print(f"Actor input shapes:")
            # print(f"observation shape: {obs.shape}")
            # print(f"state shape: {state.shape}")
            # print(f"future_state shape: {future_state.shape}")
            # print(f"goal shape: {goal.shape}")
            # print(f"concatenated observation shape: {observation.shape}")

            # Rest of the function...
            if network_id == "s":
                means, log_stds = shallow_actor.apply(actor_params, observation)
            elif network_id == "c":
                means, log_stds = collector_actor.apply(actor_params, observation)
            elif network_id == "d":
                means, log_stds = deep_actor.apply(actor_params, observation)
            else:
                raise ValueError(f"Invalid network ID: {network_id}")
            # print(f"means shape: {means.shape}")
            # print(f"log_stds shape: {log_stds.shape}")
            ...
            if network_id == "s":
                sa_repr = shallow_sa_encoder.apply(sa_encoder_params, state, action)
                g_repr = shallow_g_encoder.apply(g_encoder_params, goal)
            elif network_id == "c":
                sa_repr = collector_sa_encoder.apply(sa_encoder_params, state, action)
                g_repr = collector_g_encoder.apply(g_encoder_params, goal)
            elif network_id == "d":
                sa_repr = deep_sa_encoder.apply(sa_encoder_params, state, action)
                g_repr = deep_g_encoder.apply(g_encoder_params, goal)
            else:
                raise ValueError(f"Invalid network ID: {network_id}")
            ...
            # print(f"sa_repr shape: {sa_repr.shape}")
            # print(f"g_repr shape: {g_repr.shape}")
            # print(f"qf_pi shape: {qf_pi.shape}")
        ...
        collector_key, deep_key, shallow_key = jax.random.split(key, 3)
        
        (collector_actorloss, collector_log_prob), collector_actor_grad = jax.value_and_grad(actor_loss, has_aux=True)(training_state.collector_actor_state.params, "c", training_state.collector_critic_state.params, training_state.collector_alpha_state.params['log_alpha'], transitions, collector_key)
        new_collector_actor_state = training_state.collector_actor_state.apply_gradients(grads=collector_actor_grad)

        # print(f"*****************collector_actorloss: {collector_actorloss}")

        collector_alphaloss, collector_alpha_grad = jax.value_and_grad(alpha_loss)(training_state.collector_alpha_state.params, collector_log_prob)
        new_collector_alpha_state = training_state.collector_alpha_state.apply_gradients(grads=collector_alpha_grad)



        (deep_actorloss, deep_log_prob), deep_actor_grad = jax.value_and_grad(actor_loss, has_aux=True)(training_state.deep_actor_state.params, "d", training_state.deep_critic_state.params, training_state.deep_alpha_state.params['log_alpha'], transitions, deep_key)
        new_deep_actor_state = training_state.deep_actor_state.apply_gradients(grads=deep_actor_grad)

        deep_alphaloss, deep_alpha_grad = jax.value_and_grad(alpha_loss)(training_state.deep_alpha_state.params, deep_log_prob)
        new_deep_alpha_state = training_state.deep_alpha_state.apply_gradients(grads=deep_alpha_grad)

        (shallow_actorloss, shallow_log_prob), shallow_actor_grad = jax.value_and_grad(actor_loss, has_aux=True)(training_state.shallow_actor_state.params, "s", training_state.shallow_critic_state.params, training_state.shallow_alpha_state.params['log_alpha'], transitions, shallow_key)
        new_shallow_actor_state = training_state.shallow_actor_state.apply_gradients(grads=shallow_actor_grad)

        shallow_alphaloss, shallow_alpha_grad = jax.value_and_grad(alpha_loss)(training_state.shallow_alpha_state.params, shallow_log_prob)
        new_shallow_alpha_state = training_state.shallow_alpha_state.apply_gradients(grads=shallow_alpha_grad)

        training_state = training_state.replace(collector_actor_state=new_collector_actor_state, collector_alpha_state=new_collector_alpha_state,
                deep_actor_state=new_deep_actor_state, deep_alpha_state=new_deep_alpha_state,
                shallow_actor_state=new_shallow_actor_state, shallow_alpha_state=new_shallow_alpha_state)

        metrics = {
            "collector_sample_entropy": -collector_log_prob,
            "deep_sample_entropy": -deep_log_prob,
            "shallow_sample_entropy": -shallow_log_prob,
            "collector_actor_loss": collector_actorloss,
            "collector_alpha_loss": collector_alphaloss,
            "deep_actor_loss": deep_actorloss,
            "deep_alpha_loss": deep_alphaloss,
            "shallow_actor_loss": shallow_actorloss,
            "shallow_alpha_loss": shallow_alphaloss,
            "collector_log_alpha": training_state.collector_alpha_state.params["log_alpha"],
            "deep_log_alpha": training_state.deep_alpha_state.params["log_alpha"],
            "shallow_log_alpha": training_state.shallow_alpha_state.params["log_alpha"],
        }
    ...
    def update_critic(transitions, training_state, key):
        ...
        def critic_loss(critic_params, network_id, transitions, key):
            # Add debug prints
            # print(f"Critic input shapes:")
            # print(f"obs shape: {transitions.observation.shape}")
            ...
            if network_id == "s":
                sa_repr = shallow_sa_encoder.apply(sa_encoder_params, obs, action)
                g_repr = shallow_g_encoder.apply(g_encoder_params, transitions.observation[:, args.obs_dim:])
            elif network_id == "c":
                sa_repr = collector_sa_encoder.apply(sa_encoder_params, obs, action)
                g_repr = collector_g_encoder.apply(g_encoder_params, transitions.observation[:, args.obs_dim:])
            elif network_id == "d":
                sa_repr = deep_sa_encoder.apply(sa_encoder_params, obs, action)
                g_repr = deep_g_encoder.apply(g_encoder_params, transitions.observation[:, args.obs_dim:])
            else:
                raise ValueError(f"Invalid network ID: {network_id}")
            ...
            # After computing representations
        ...
        (collector_loss, (collector_logsumexp, collector_I, collector_correct, collector_logits_pos, collector_logits_neg, collector_new_memory_bank_state)), \
            collector_grad = jax.value_and_grad(critic_loss, has_aux=True)(training_state.collector_critic_state.params, "c", transitions, collector_key)
        new_collector_critic_state = training_state.collector_critic_state.apply_gradients(grads=collector_grad)

        (deep_loss, (deep_logsumexp, deep_I, deep_correct, deep_logits_pos, deep_logits_neg, deep_new_memory_bank_state)), deep_grad = jax.value_and_grad(critic_loss, has_aux=True)(training_state.deep_critic_state.params, "d", transitions, deep_key)
        new_deep_critic_state = training_state.deep_critic_state.apply_gradients(grads=deep_grad)

        (shallow_loss, (shallow_logsumexp, shallow_I, shallow_correct, shallow_logits_pos, shallow_logits_neg, shallow_new_memory_bank_state)), shallow_grad = jax.value_and_grad(critic_loss, has_aux=True)(training_state.shallow_critic_state.params, "s", transitions, shallow_key)
        new_shallow_critic_state = training_state.shallow_critic_state.apply_gradients(grads=shallow_grad)
    
        training_state = training_state.replace(collector_critic_state = new_collector_critic_state, memory_bank_state=collector_new_memory_bank_state,
                deep_critic_state = new_deep_critic_state,
                shallow_critic_state = new_shallow_critic_state)

        metrics = {
            "categorical_accuracy": jnp.mean(collector_correct),
            "collector_logits_pos": collector_logits_pos,
            "collector_logits_neg": collector_logits_neg,
            "collector_logsumexp": collector_logsumexp.mean(),
            "collector_critic_loss": collector_loss,
            "deep_logits_pos": deep_logits_pos,
            "deep_logits_neg": deep_logits_neg,
            "deep_logsumexp": deep_logsumexp.mean(),
            "deep_critic_loss": deep_loss,
            "shallow_logits_pos": shallow_logits_pos,
            "shallow_logits_neg": shallow_logits_neg,
            "shallow_logsumexp": shallow_logsumexp.mean(),
            "shallow_critic_loss": shallow_loss,
        }
    ...
    def training_step(training_state, env_state, buffer_state, key, t):
        ...
        # Sample transitions and process them...
        transitions_list = []
        ...
        # print(f"Before scan - transitions shape: {transitions.observation.shape}")
        
        (training_state, _), batch_metrics = jax.lax.scan(sgd_step, (training_state, training_key), transitions)
        
        # print(f"After scan - batch_metrics: {batch_metrics}")
        
        return (training_state, env_state, buffer_state,), batch_metrics  # Return batch_metrics instead of undefined metrics
    ...
    # TODO: deal with eval later and pass with network_id
    if args.eval_actor == 0:
        '''Setting up evaluator'''
        collector_evaluator = CrlEvaluator(
            lambda training_state, env, env_state, extra_fields: deterministic_actor_step(
                training_state, 
                "c",  # or "d" for deep or "s" for shallow
                env, 
                env_state, 
                extra_fields
            ),
        ...
        deep_evaluator = CrlEvaluator(
            lambda training_state, env, env_state, extra_fields: deterministic_actor_step(
                training_state, 
                "d",  # or "d" for deep or "s" for shallow
                env, 
                env_state, 
                extra_fields
            ),
        ...
        shallow_evaluator = CrlEvaluator(
            lambda training_state, env, env_state, extra_fields: deterministic_actor_step(
                training_state, 
                "s",  # or "d" for deep or "s" for shallow
                env, 
                env_state, 
                extra_fields
            ),
    ...
    def get_specific_metrics(metrics, network_specific_metrics, network_id):
        network_specific_keys = set(network_specific_metrics.keys())
        metrics_keys = set(metrics.keys())
        extra_keys = network_specific_keys - metrics_keys
        newDict = {}
        for key in extra_keys:
            ind = key.find("/")
            if ind != -1:
                newDict[key[:ind] + "/" + network_id + "_" + key[ind+1:]] = network_specific_metrics[key]
            else:
                newDict[network_id + "_" + key] = network_specific_metrics[key]
        return newDict
    ...
    for ne in range(args.num_epochs):
        ...
        collector_metrics = collector_evaluator.run_evaluation(training_state, metrics)
        deep_metrics = deep_evaluator.run_evaluation(training_state, metrics)
        shallow_metrics = shallow_evaluator.run_evaluation(training_state, metrics)

        collector_metrics = get_specific_metrics(metrics, collector_metrics, "c")
        deep_metrics = get_specific_metrics(metrics, deep_metrics, "d")
        shallow_metrics = get_specific_metrics(metrics, shallow_metrics, "s")

        metrics = {**metrics, **collector_metrics, **deep_metrics, **shallow_metrics}
    ...
    if args.capture_vis:
        def render_policy(training_state, network_id, save_path):
            """Renders the policy and saves it as an HTML file."""
            # JIT compile the rollout function
            @jax.jit
            def policy_step(env_state, actor_params):
                if network_id == "s":
                    means, _ = shallow_actor.apply(actor_params, env_state.obs)
                elif network_id == "c":
                    means, _ = collector_actor.apply(actor_params, env_state.obs)
                elif network_id == "d":
                    means, _ = deep_actor.apply(actor_params, env_state.obs)
                else:
                    raise ValueError(f"Invalid network ID: {network_id}")
                actions = nn.tanh(means)
            ...
            for i in range(8):
                ...
                for _ in range(1000):
                    if network_id == "c":
                        env_state, current_state = policy_step(env_state, training_state.collector_actor_state.params)
                    elif network_id == "d":
                        env_state, current_state = policy_step(env_state, training_state.deep_actor_state.params)
                    elif network_id == "s":
                        env_state, current_state = policy_step(env_state, training_state.shallow_actor_state.params)
                    rollout_states.append(current_state.pipeline_state)
            
            # Render and save
            html_string = html.render(env.sys, rollout_states)
            render_path = Path(save_path) / f"vis_{network_id}.html"
            render_path.parent.mkdir(parents=True, exist_ok=True)
            with open(render_path, "w") as f:
                f.write(html_string)
            wandb.log({f"vis_{network_id}": wandb.Html(html_string)})
            
        print("Rendering final policy...", flush=True)
        try:
            render_policy(training_state, "c", save_path)
            render_policy(training_state, "d", save_path)
            render_policy(training_state, "s", save_path)
