...
class ResidualBlock(nn.Module):
    ...
    def __call__(self, carry, _):
        ...
        identity = carry
        x = carry
        # Apply 4 Dense layers with normalization and activation.
        for _ in range(4):
            x = nn.Dense(self.network_width,
                         kernel_init=self.lecun_uniform,
                         bias_init=self.bias_init)(x)
            x = self.normalize(x)
            x = self.activation(x)
        # Add the residual connection.
        new_carry = x + identity
        # We return None as the auxiliary output (you could output something else if needed).
        return new_carry, None
    
class SA_encoder(nn.Module):
    norm_type: str = "layer_norm"
    network_width: int = 1024
    network_depth: int = 4  # must be a multiple of 4
    skip_connections: int = 0  # not used in this example
    use_relu: bool = False

    @nn.compact
    def __call__(self, s: jnp.ndarray, a: jnp.ndarray):
        # Define your initializers.
        ...
        ScannedResidualBlock = nn.scan(
            ResidualBlock,
            variable_broadcast="params",   # share parameters across iterations
            split_rngs={"params": False},    # do not split the rng for the shared parameters
            in_axes=0,                      # scan over the first axis of the dummy input
            out_axes=0,                     # stack outputs along the first axis
            length=num_blocks               # number of iterations
        )

        # Create a dummy input to drive the scan. Its value is not used.
        dummy = jnp.zeros((num_blocks,))

        # Call the scanned module. Here, x is the carry state.
        # The scan will run for num_blocks iterations.
        x, _ = ScannedResidualBlock(
...
class G_encoder(nn.Module):
    ...
    def __call__(self, g: jnp.ndarray):
        ...
        # Use nn.scan to apply the residual block repeatedly.
        num_blocks = self.network_depth // 4
        dummy = jnp.zeros((num_blocks,))  # dummy input for the scan

        ScannedResidualBlock = nn.scan(
            ResidualBlock,  # Assumes ResidualBlock is defined elsewhere.
            variable_broadcast="params",   # share parameters across iterations
            split_rngs={"params": False},    # do not split the RNG for parameters
            in_axes=0,                      # iterate over the dummy's first axis
            out_axes=0,                     # stack outputs along the first axis
            length=num_blocks               # number of residual block iterations
        )
...
class Actor(nn.Module):
    ...
    def __call__(self, x):
        ...
        # Residual blocks using nn.scan.
        num_blocks = self.network_depth // 4
        dummy = jnp.zeros((num_blocks,))  # dummy input for scan
        ...
        # Final layers: one Dense for the mean, one for the log_std.
