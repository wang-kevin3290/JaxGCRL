...
class Args:
    ...
    state_encoder_width: int = 256
    state_encoder_depth: int = 4
    latent_model_width: int = 256
    latent_model_depth: int = 4
    actor_width: int = 256
    actor_depth: int = 4
    
    # Remove old network width/depth arguments
    # network_width: int = 256
    # network_depth: int = 4
    
    # Keep existing skip connections parameters
...
class Latent_space_model(nn.Module):
    norm_type = "layer_norm"
    network_width: int = 256
    network_depth: int = 4
    skip_connections: int = 4
    ...
    @nn.compact
    def __call__(self, encoded_state: jnp.ndarray, action: jnp.ndarray):
        x = jnp.concatenate([encoded_state, action], axis=-1)
...
if __name__ == "__main__":
    ...
    # Network setup
    # Actor
    actor = Actor(action_size=action_size, network_width=args.actor_width, network_depth=args.actor_depth, skip_connections=args.actor_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    ...
    # Critic
    state_encoder = State_encoder(network_width=args.state_encoder_width, network_depth=args.state_encoder_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    state_encoder_params = state_encoder.init(sa_key, np.ones([1, args.obs_dim]))
    latent_space_model = Latent_space_model(network_width=args.latent_model_width, network_depth=args.latent_model_depth, skip_connections=args.critic_skip_connections, use_relu=args.use_relu, resnet_type=args.resnet)
    latent_space_model_params = latent_space_model.init(g_key, np.ones([1, 64]), np.ones([1, action_size]))
    ...
    if not args.mrn:
        critic_state = TrainState.create(
            ...
            params={
                ...
                "latent_space_model": latent_space_model_params
            },
    ...
    def multi_sample_actor_step(training_state, env, env_state, key, K, extra_fields):
        ...
        # First encode states
        s_encoded = state_encoder.apply(
            training_state.critic_state.params["state_encoder"], 
            state
        )  # Shape: (batch_size, repr_dim)
        
        # Then apply latent space model to encoded states + actions
        sa_reprs = jax.vmap(
            lambda a: latent_space_model.apply(
                training_state.critic_state.params["latent_space_model"],
                s_encoded,
                a
            )
        )(actions)  # Shape: (K, batch_size, repr_dim)
        
        g_repr = state_encoder.apply(
            training_state.critic_state.params["state_encoder"], 
            goal
        )  # Shape: (batch_size, repr_dim)
    ...
    def update_actor_and_alpha(transitions, training_state, key):
        ...
        def actor_loss(actor_params, critic_params, log_alpha, transitions, key):
            ...
            # Use the full future state as the goal, since it contains complete state information
            goal_obs = transitions.extras["future_state"]  # This already has the full state dimension
            
            # Create observation by concatenating current state and goal state
            observation = jnp.concatenate([state, goal_obs], axis=1)
            ...
            # First encode state
            sa_repr = state_encoder.apply(critic_params["state_encoder"], state)
            
            # Then apply latent space model to encoded state + action
            sa_repr = latent_space_model.apply(critic_params["latent_space_model"], sa_repr, action)
            
            # Encode goal using the full future state
            g_repr = state_encoder.apply(critic_params["state_encoder"], goal_obs)
    ...
    def update_critic(transitions, training_state, key):
        ...
        def critic_loss(critic_params, transitions, key):
            state_encoder_params = critic_params["state_encoder"]
            latent_space_model_params = critic_params["latent_space_model"]
            
            obs = transitions.observation[:, :args.obs_dim]
            action = transitions.action
            
            # First encode state
            state_repr = state_encoder.apply(state_encoder_params, obs)
            
            # Then apply latent space model to encoded state + action
            sa_repr = latent_space_model.apply(latent_space_model_params, state_repr, action)
            
            # Encode goal separately
            goal_repr = state_encoder.apply(state_encoder_params, transitions.observation[:, args.obs_dim:])

            if args.memory_bank:
                new_memory_bank_state, (sa_bank, g_bank) = memory_bank.sample(training_state.memory_bank_state)
                sa_repr = jnp.concatenate([sa_repr, sa_bank], axis=0)
                g_repr = jnp.concatenate([goal_repr, g_bank], axis=0)
    ...
    def training_step(training_state, env_state, buffer_state, key, t):
        ...
        # Fix: Add lambda function to tree_map
        transitions = jax.tree_util.tree_map(
